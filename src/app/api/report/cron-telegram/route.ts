 ...
 * 
 * Endpoint untuk auto-trigger laporan harian ke Telegram.
 * Auth via API key (header Authorization: Bearer <REPORT_API_KEY>) atau internal call.
 * Bisa dipanggil oleh:
 * - Internal scheduler (instrumentation.ts)
 * - External cron (curl/wget)
 * - Vercel Cron
 */
export async function GET(request: NextRequest) {
    // Auth via API Key OR internal secret header
    const apiKey = process.env.REPORT_API_KEY
    const authHeader = request.headers.get('authorization')
    const token = authHeader?.replace('Bearer ', '').trim()
    const internalSecret = request.headers.get('x-internal-cron')

    // Allow if: valid API key OR internal cron secret matches
    const isAuthorized = (apiKey && token === apiKey) || (internalSecret === (process.env.CRON_SECRET || 'elyasr-internal-cron'))
    ...
    const autoEnabled = await getSetting('auto_report_enabled')
    if (autoEnabled === 'false') {
        return NextResponse.json({
            success: false,
            error: 'Auto report is disabled',
            skipped: true
        })
    }
    ...
    if (!botToken || !chatId) {
        return NextResponse.json({
            success: false,
            error: 'Konfigurasi Telegram belum diisi.',
        }, { status: 400 })
    }
...
<i>⏰ Dikirim otomatis jam 17.30 WIB dari Elyasr Ops</i>`
        ...
        return NextResponse.json({
            success: true,
            message: 'Laporan otomatis berhasil dikirim ke Telegram!',
            date: dateFrom,
