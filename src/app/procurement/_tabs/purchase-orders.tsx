...
function POItemSelect({ item, onSelect }: { item: any; onSelect: (product: any) => void }) {
  ...
  return (
    <div className="relative flex-1" ref={dropdownRef}>
      <button type="button" onClick={() => { setOpen(true); setSearchQuery(''); setSuggestions([]) }}
        className="w-full text-left bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-200 focus:outline-none flex justify-between items-center hover:border-zinc-600">
      ...
      {open && (
        <div className="absolute z-50 w-full mt-1 bg-zinc-800 border border-zinc-700 rounded-lg shadow-xl overflow-hidden">
          <div className="p-2 border-b border-zinc-700">
            <input autoFocus value={searchQuery} onChange={e => { setSearchQuery(e.target.value); fetchSuggestions(e.target.value) }}
          ...
          <div className="max-h-52 overflow-y-auto divide-y divide-zinc-700/50">
            {loadingSuggest && <p className="text-center py-3 text-xs text-zinc-500 animate-pulse">Mencari...</p>}
            {!loadingSuggest && suggestions.map(p => (
              <button key={p.sku} type="button" onClick={() => handleSelect(p)}
                className={`w-full text-left px-3 py-2 text-xs hover:bg-zinc-700 ${item.sku === p.sku ? 'bg-emerald-900/30 text-emerald-300' : 'text-zinc-300'}`}>
                <span className="font-mono text-emerald-400/80 mr-1.5">{p.sku}</span>— {p.productName}
              </button>
            ))}
            {!loadingSuggest && searchQuery.length > 0 && suggestions.length === 0 && <p className="text-center py-3 text-xs text-zinc-500">Produk tidak ditemukan</p>}
            {!loadingSuggest && searchQuery.length === 0 && <p className="text-center py-3 text-xs text-zinc-500">Ketik untuk mencari produk...</p>}
          </div>
        </div>
      )}
    </div>
...
function POFormModal({ vendors, editPO, onClose }: { vendors: any[]; editPO: any | null; onClose: () => void }) {
  ...
  const [items, setItems] = useState<{sku:string;productName?:string;categoryName?:string;qtyOrder:number}[]>(
    isEdit ? (editPO.items?.map((it:any)=>({sku:it.sku,productName:it.productName,categoryName:'',qtyOrder:it.qtyOrder})) || [{sku:'',qtyOrder:1}]) : [{sku:'',qtyOrder:1}]
  )
  ...
  // Hitung preview split berdasarkan kategori
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (autoSplit && isMultiCategory && !isEdit) {
        ...
        const payload = { vendorId, poDate, expectedDate: expectedDate||null, note, items: validItems.map(i=>({sku:i.sku,qtyOrder:i.qtyOrder})), ...(poNumberOverride.trim() && !isEdit ? {poNumberOverride:poNumberOverride.trim()} : {}) }
        const res = isEdit
      ...
      qc.invalidateQueries({ queryKey:['purchase-orders'] }); onClose()
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-2xl p-6 pb-8 max-h-[92vh] overflow-y-auto">
        <h2 className="text-base font-semibold text-white mb-5">{isEdit ? `Edit PO — ${editPO.poNumber}` : 'Buat Purchase Order'}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-zinc-500 mb-1">No. PO {!isEdit && <span className="text-zinc-600">(kosong = auto)</span>}</label>
              <input value={poNumberOverride} onChange={e => setPoNumberOverride(e.target.value)} disabled={isEdit}
            ...
            <div className="space-y-2">
              {items.map((item, i) => (
                <div key={i} className="flex gap-2">
                  <POItemSelect item={item} onSelect={(p) => setItems(prev => prev.map((it, idx) => idx===i ? {...it, sku:p.sku, productName:p.productName, categoryName:p.categoryName||''} : it))} />
                  ...
                  {items.length > 1 && <button type="button" onClick={() => removeItem(i)} className="text-zinc-600 hover:text-red-400 px-2">✕</button>}
                </div>
              ))}
            </div>
          </div>
          {/* Auto Split Toggle — hanya mode create & ada item valid */}
      ...
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
        <div className="flex items-start justify-between px-6 py-4 border-b border-zinc-800">
          ...
          <div className="flex items-start gap-6 text-right">
            <div><p className="text-xs text-zinc-500">Tanggal:</p><p className="text-xs text-zinc-200">{formatDate(po.poDate)}</p></div>
            <div><p className="text-xs text-zinc-500">Progress</p><p className="text-xs text-zinc-200 font-bold">{progressPct}% <span className="text-zinc-500 font-normal">{po.totalQtyReceived}/{po.totalQtyOrder}</span></p></div>
            <button onClick={onClose} className="text-zinc-500 hover:text-white"><X size={18} /></button>
          </div>
        </div>
        <div className="grid grid-cols-3 divide-x divide-zinc-800 border-b border-zinc-800">
          <div className="px-6 py-3"><p className="text-xs text-zinc-500 mb-1">Total PO</p><p className="text-sm font-bold text-white">{formatRupiah(po.totalAmount, true)}</p></div>
          <div className="px-6 py-3"><p className="text-xs text-zinc-500 mb-1">Terbayar</p><p className="text-sm font-bold text-emerald-400">{formatRupiah(po.totalPaid, true)}</p></div>
          <div className="px-6 py-3"><p className="text-xs text-zinc-500 mb-1">Outstanding</p><p className="text-sm font-bold text-amber-400">{formatRupiah(totalOutstanding, true)}</p></div>
        </div>
        <div className="flex-1 overflow-auto">
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-zinc-800/80">
              <tr>
                {['SKU','Produk','HPP','Order','Received','Outstanding','Subtotal','Status'].map(h => (
              ...
              {(po.items || []).map((item:any) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-zinc-800/50">
                    ...
                    <td className="px-4 py-2 text-right font-bold text-zinc-200">{item.qtyOrder}</td>
                    ...
                    <td className="px-4 py-2 text-right text-zinc-200">{formatRupiah(item.qtyOrder * item.unitPrice, true)}</td>
                    <td className="px-4 py-2 text-center"><span className={outstanding>0?'badge-warning text-[10px]':'badge-success text-[10px]'}>{outstanding>0?'OPEN':'DONE'}</span></td>
                  </tr>
                )
              })}
            </tbody>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-zinc-800">
          ...
          <div className="flex gap-2">
            <button onClick={() => onDownload(po)} className="flex items-center gap-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-blue-400 rounded-lg px-3 py-1.5 text-xs border border-zinc-700"><FileDown size={13}/>Download CSV</button>
            <button onClick={() => onPrint(po)} className="flex items-center gap-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-emerald-400 rounded-lg px-3 py-1.5 text-xs border border-zinc-700"><Printer size={13}/>Cetak PDF</button>
            <button onClick={onClose} className="bg-zinc-700 hover:bg-zinc-600 text-zinc-300 rounded-lg px-4 py-1.5 text-xs">Tutup</button>
          </div>
        </div>
      </div>
    </div>
...
function PrintPOModal({ po, onClose }: { po:any; onClose:()=>void }) {
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <style>{`@media print{body *{visibility:hidden}#printable-po,#printable-po *{visibility:visible}#printable-po{position:absolute;left:0;top:0;width:100%;background:white!important;color:black!important;padding:30px;font-family:sans-serif}.no-print{display:none!important}}`}</style>
      <div className="bg-white rounded-xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b bg-zinc-50 border-zinc-200 no-print">
          ...
          <div className="flex gap-2">
            <button onClick={() => window.print()} className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded px-4 py-1.5 text-sm font-medium"><Printer size={14}/>Print</button>
            <button onClick={onClose} className="p-1.5 rounded bg-zinc-200 hover:bg-zinc-300 text-zinc-600">✕</button>
          </div>
        </div>
        <div className="flex-1 overflow-auto p-8 bg-white text-zinc-900" id="printable-po">
          <h1 className="text-2xl font-bold text-center mb-8 uppercase tracking-widest">Purchase Order</h1>
          <div className="flex justify-between mb-8">
            <div className="space-y-1">
              ...
              <p className="text-sm"><span className="font-bold">Estimasi Tiba:</span> {po.expectedDate ? formatDate(po.expectedDate) : '-'}</p>
            </div>
          ...
          <table className="w-full mb-8 border-collapse border border-zinc-800">
            <thead>
              <tr className="bg-zinc-100 border-b border-zinc-800">
                {['No','SKU','Nama Produk','Qty Order'].map(h => <th key={h} className="text-left py-2 px-3 text-xs font-bold border-r border-zinc-800 last:border-r-0">{h}</th>)}
              </tr>
            </thead>
            <tbody>
          ...
          <div className="mb-12">
            ...
            <div className="border border-zinc-300 rounded p-4 text-sm min-h-[60px]">{po.note||'-'}</div>
          </div>
          <div className="flex justify-between px-16 mt-16">
            <div className="text-center"><p className="text-sm mb-20 font-bold">Vendor</p><div className="w-32 border-t border-black mx-auto"></div></div>
            <div className="text-center"><p className="text-sm mb-20 font-bold">Purchasing</p><div className="w-32 border-t border-black mx-auto"></div></div>
          </div>
        </div>
      </div>
    </div>
...
export function PurchaseOrdersTab() {
  ...
  const handleDownload = (po: any) => {
    downloadCSV(`PO-${po.poNumber}.csv`, (po.items||[]).map((item:any) => ({
      'Kode PO': po.poNumber, SKU: item.sku, 'Nama Artikel': item.productName,
      'Qty Order': item.qtyOrder, 'Qty Receive': item.qtyReceived||0, HPP: item.unitPrice, Subtotal: item.qtyOrder * item.unitPrice
    })))
  }

  const handleDelete = async (po: any) => {
    const isRequestOnly = isFinance
    if (!confirm(isRequestOnly ? `Request delete PO ${po.poNumber}?` : `Hapus PO ${po.poNumber}?`)) return
    setDeletingId(po.id)
    try {
      const res  = await fetch('/api/purchase-orders', { method:'DELETE', headers:{'Content-Type':'application/json'}, body:JSON.stringify({id:po.id,requestOnly:isRequestOnly}) })
      const json = await res.json()
      if (!res.ok) throw new Error(json.error)
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex gap-3 flex-1">
          <div className="relative flex-1 max-w-sm">
            ...
            <input value={search} onChange={e => { setSearch(e.target.value); setPage(1) }} placeholder="Cari no. PO atau vendor..."
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="data-table">
            <thead><tr>
              <th className="w-36">No. PO</th><th>Vendor</th><th className="w-24">Tgl PO</th>
              <th className="w-20 text-center">Items</th><th className="w-28 text-right">Total</th>
              <th className="w-28 text-right">Terbayar</th><th className="w-24">Status</th>
              <th className="w-20 text-center">Diterima</th><th className="w-24 text-center">Aksi</th>
            </tr></thead>
            <tbody>
                ...
                return (
                  <tr key={po.id} className={isDeleteReq ? 'bg-red-950/20' : ''}>
                    <td>
                      ...
                      {isDeleteReq && <span className="ml-1.5 text-[9px] text-red-400 bg-red-900/30 border border-red-900/40 px-1.5 py-0.5 rounded">DELETE REQ</span>}
                    </td>
                    ...
                    <td>
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => setDetailPO(po)} className="p-1.5 rounded bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-white" title="Detail"><Eye size={13}/></button>
                        ...
                        {(isOwner || isFinance) && (
                          <button onClick={() => handleDelete(po)} disabled={deletingId===po.id}
                            className={`p-1.5 rounded disabled:opacity-40 ${isOwner?'bg-zinc-800 hover:bg-red-900/40 text-zinc-400 hover:text-red-400':'bg-zinc-800 hover:bg-orange-900/40 text-zinc-400 hover:text-orange-400'}`}
