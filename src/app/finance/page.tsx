...
import { Wallet, Plus, Download, Settings, ChevronLeft, ChevronRight, HelpCircle, X, ChevronDown, Search } from 'lucide-react'
...
const TRX_TYPES = [
  { value: 'EXPENSE',      label: 'Beban Operasional',    tooltip: 'Beban operasional bisnis (listrik, gaji, dll). Masuk laporan P&L.' },
  { value: 'OTHER_INCOME', label: 'Pendapatan Lain',      tooltip: 'Pendapatan selain penjualan marketplace.' },
  { value: 'MODAL_MASUK',  label: 'Suntikan Modal',       tooltip: 'Suntikan modal dari pemilik. Tidak masuk P&L, masuk Balance Sheet (menambah ekuitas).' },
  { value: 'PRIVE',        label: 'Prive (Ambil Modal)',  tooltip: 'Pengambilan modal oleh pemilik. Mengurangi ekuitas.' },
  { value: 'INVESTASI',    label: 'Pembelian Aset Tetap', tooltip: 'Pembelian aset tetap (laptop, kendaraan, dll). Masuk Balance Sheet.' },
  { value: 'TRANSFER',     label: 'Transfer Antar Wallet', tooltip: 'Pindah dana antar wallet. Tidak masuk laporan.' },
]

// ── Tooltip komponen kecil ────────────────────────────────────────────
function Tooltip({ text }: { text: string }) {
  ...
  return (
    <span className="relative inline-block ml-1">
      ...
      />
      {show && (
        <div className="absolute z-50 bottom-5 left-1/2 -translate-x-1/2 bg-zinc-800 border border-zinc-700 text-zinc-300 text-xs rounded-lg px-3 py-2 w-56 shadow-xl pointer-events-none">
          {text}
        </div>
      )}
    </span>
  ...
  return (
    <div className="relative">
      <button
        ...
        className="w-full flex items-center justify-between bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-left hover:border-zinc-600 transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
      >
        <span className={selected ? 'text-zinc-200' : 'text-zinc-500'}>
          {selected ? selected.name : 'Cari atau pilih kategori...'}
        </span>
        ...
        <div className="absolute z-50 w-full mt-1 bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl overflow-hidden">
          <div className="p-2 border-b border-zinc-800">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-zinc-800 border border-zinc-700 rounded-lg pl-8 pr-3 py-1.5 text-xs text-zinc-200 placeholder-zinc-600 focus:outline-none"
              />
            </div>
          </div>
            ...
            {filtered ? (
              filtered.map((c) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-xs hover:bg-zinc-800 transition-colors ${value === c.name ? 'bg-emerald-900/30 text-emerald-300' : 'text-zinc-300'}`}
                >
              ...
              Object.entries(grouped).map(([grp, cats]) => (
                <div key={grp}>
                  <div className="px-3 py-1.5 text-[10px] font-bold text-zinc-500 uppercase tracking-wide bg-zinc-800/50 border-t border-zinc-800 first:border-t-0">
                    {grp}
                  </div>
                  {cats.map((c: any) => (
                    <button
                      ...
                      type="button"
                      onClick={() => { onChange(c.name); setOpen(false); setSearch('') }}
                      className={`w-full text-left px-4 py-2 text-xs hover:bg-zinc-800 transition-colors ${value === c.name ? 'bg-emerald-900/30 text-emerald-300' : 'text-zinc-300'}`}
                    >
                      {c.name}
                    </button>
                  ))}
          ...
          <div className="border-t border-zinc-800">
            <button
              ...
              className="w-full text-left px-3 py-2.5 text-xs text-emerald-400 hover:bg-zinc-800 transition-colors flex items-center gap-1.5"
            >
              <Plus size={11} />
...
// ── Mini modal tambah kategori baru ──────────────────────────────────
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            ...
            <input
              ...
              required
              placeholder="cth: Komisi Reseller"
...
// ── Modal Tambah Transaksi (versi baru) ─────────────────────────────
    ...
    <>
      {showAddCat && (
        <AddCategoryModal
          ...
          onCreated={(name) => { set('category', name); qc.invalidateQueries({ queryKey: ['expense-categories'] }) }}
        />
      )}
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {/* Tipe */}
            <div>
              <label className="block text-xs text-zinc-500 mb-1 flex items-center">
                Tipe
                {selectedType && <Tooltip text={selectedType.tooltip} />}
              </label>
            ...
            {/* Kategori - hanya EXPENSE pakai dropdown terstruktur */}
            {form.trxType !== 'TRANSFER' && (
              <div>
                <label className="block text-xs text-zinc-500 mb-1 flex items-center">
                  Kategori
                  {form.trxType === 'EXPENSE' && (
                    <Tooltip text="Pilih kategori beban. Kategori baru bisa ditambah langsung dari dropdown." />
                  )}
                </label>
                {form.trxType === 'EXPENSE' ? (
                  <CategoryDropdown
            ...
            <div>
              <label className="block text-xs text-zinc-500 mb-1 flex items-center">
                Jumlah (Rp)
                <Tooltip text="Masukkan nominal tanpa tanda minus. Sistem otomatis menentukan arah berdasarkan tipe transaksi." />
              </label>
              ...
              {form.amount && (
                <p className="text-[10px] text-zinc-600 mt-1">
                  {['EXPENSE','PRIVE','INVESTASI'].includes(form.trxType) ? '← Keluar' : '→ Masuk'} wallet ·{' '}
                  {formatRupiah(parseInt(form.amount) || 0, true)}
                </p>
              )}
            </div>
...
function ManageWalletsModal({ onClose, wallets }: { onClose: () => void; wallets: any[] }) {
  ...
  const toggleActive = async (id: string, currentStatus: boolean) => {
    if (!confirm(`Yakin ingin ${currentStatus ? 'menonaktifkan' : 'mengaktifkan'} wallet ini?`)) return
    setLoading(true)
    try {
      await fetch(`/api/wallet/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ isActive: !currentStatus }) })
      qc.invalidateQueries({ queryKey: ['wallets'] })
  ...
  return (
    <AppLayout>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
        {(wallets ?? []).map((w: any) => (
          <div key={w.id}
            className={`stat-card cursor-pointer transition-all ${walletFilter === w.id ? 'ring-2 ring-emerald-500/50' : ''}`}
      ...
      <div className="flex gap-3 mb-4 flex-wrap">
        ...
        {walletFilter && (
          <button onClick={() => setWalletFilter('')} className="text-xs text-zinc-500 hover:text-zinc-300">✕ Reset filter wallet</button>
        )}
      </div>
