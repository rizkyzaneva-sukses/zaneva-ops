...
function VendorModal({ vendor, onClose }: { vendor?: any; onClose: () => void }) {
  ...
  const [form, setForm] = useState({
    vendorCode: vendor?.vendorCode ?? '', namaVendor: vendor?.namaVendor ?? '',
    kontak: vendor?.kontak ?? '', email: vendor?.email ?? '',
    alamat: vendor?.alamat ?? '', rekening: vendor?.rekening ?? '',
    bank: vendor?.bank ?? '', termPayment: vendor?.termPayment ?? 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      qc.invalidateQueries({ queryKey: ['vendors'] }); qc.invalidateQueries({ queryKey: ['vendors-all'] }); onClose()
  ...
  const fields = [
    ...
    { label: 'Nama Vendor *', key: 'namaVendor' }, { label: 'Kontak', key: 'kontak' },
    { label: 'Email', key: 'email' },        { label: 'Rekening', key: 'rekening' },
    { label: 'Bank', key: 'bank' },          { label: 'Alamat', key: 'alamat' },
  ]
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          {fields.map(f => (
            <div key={f.key}>
              ...
              <input value={(form as any)[f.key]} onChange={e => set(f.key, e.target.value)} disabled={f.disabled}
          ...
          {isEdit && (
            <div className="flex items-center gap-2">
              <input type="checkbox" id="va2" checked={form.isActive} onChange={e => set('isActive', e.target.checked)} className="rounded" />
              <label htmlFor="va2" className="text-xs text-zinc-400">Aktif</label>
            </div>
          )}
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => setModal('add')} className="flex items-center gap-2 bg-emerald-700 hover:bg-emerald-600 text-white rounded-lg px-3 py-2 text-sm font-medium ml-3">
          ...
          <tbody>
            {isLoading ? Array.from({length:5}).map((_,i) => (
              ...
              <tr key={v.id}>
                ...
                <td>
                  <button onClick={() => { setSelected(v); setModal('edit') }} className="p-1.5 rounded hover:bg-zinc-800 text-zinc-600 hover:text-zinc-300">
                    <Edit2 size={12}/>
                  </button>
                </td>
              </tr>
