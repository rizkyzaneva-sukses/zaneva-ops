...
import { ClipboardCheck, Upload, Download, CheckCircle, Loader2, AlertTriangle } from 'lucide-react'
...
export default function StockOpnamePage() {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const json = await res.json()
      if (!res.ok) throw new Error(json.error)
      toast({ title: 'Opname berhasil dicommit! Stok telah diperbarui.', type: 'success' })
      setPreview(null)
  ...
  const downloadTemplate = () => {
    downloadCSV('template-opname.csv', [
      { SKU: 'CONTOH-SKU-001', New_SOH: 100 },
      { SKU: 'CONTOH-SKU-002', New_SOH: 50 },
    ])
  }
  ...
  return (
    <AppLayout>
      <div className="page-header">
        <h1 className="page-title flex items-center gap-2"><ClipboardCheck size={22} className="text-emerald-400"/>Stock Opname</h1>
        <div className="flex gap-2">
          <button onClick={downloadTemplate} className="flex items-center gap-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg px-3 py-2 text-sm border border-zinc-700 transition-colors">
      ...
      <div className="mb-4 text-xs text-zinc-600 bg-zinc-900/50 border border-zinc-800 border-dashed rounded-lg px-3 py-2">
        ...
        <div className="bg-zinc-900 border border-emerald-800/50 rounded-xl p-5 mb-6">
          ...
          <div className="max-h-64 overflow-y-auto">
            <table className="data-table w-full">
              <thead>
                <tr><th>SKU</th><th className="w-24 text-center">SOH Sistem</th><th className="w-24 text-center">SOH Aktual</th><th className="w-24 text-center">Selisih</th></tr>
              </thead>
              <tbody>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
        ...
        <table className="data-table w-full">
          <thead>
            <tr><th className="w-28">Tanggal</th><th className="w-24 text-center">Total SKU</th><th className="w-28 text-center">Total Adj.</th><th className="w-24">Status</th><th className="w-28">Dicommit oleh</th></tr>
          </thead>
          <tbody>
